...
public class CompactionTask extends AbstractCompactionTask
{
    ...
    protected void runMayThrow() throws Exception
    {
        ...
        try (CompactionController controller = getCompactionController(transaction.originals()))
        {
            ...
            int bytesPerSeconds = FBUtilities.bytesPerSeconds(startsize-endsize, durationInNano);
            ...
            logger.debug(String.format("Compacted (%s) %d sstables to [%s] to level=%d.  %s to %s (~%d%% of original) in %,dms = %s.  %,d total partitions merged to %,d.  Partition merge counts were {%s}",
                                      taskId,
                                      ...
                                      (int) (ratio * 100),
                                      dTime,
                                      FBUtilities.prettyPrintRateInSeconds(bytesPerSeconds),
                                      totalSourceRows,
                                      totalKeysWritten,
                                      mergeSummary));
